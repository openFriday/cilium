...
// getNets gets every cidr associated with an identity.
// This includes generated cidrs associated with this identity,
// which mean that this slice can easily be dozens of subnets large.
			...
			if !v.IsDeny {
				if entryIdentityIsSupersetOf(k, v, newKey, newEntry, identities) {
					if newKey.PortProtoIsBroader(k) {
						...
						// specific port-porotocol of the iterated-allow-entry must be inserted.
						newKeyCpy := newKey
			...
			if v.IsDeny {
				if entryIdentityIsSupersetOf(newKey, newEntry, k, v, identities) {
					if k.PortProtoIsBroader(newKey) {
						// If the new-entry is a *only* superset of the iterated-deny-entry
